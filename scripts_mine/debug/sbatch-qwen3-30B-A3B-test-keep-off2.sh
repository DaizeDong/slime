#!/bin/bash
#SBATCH --job-name=debug-keep-off2
#SBATCH --nodes=1
...
if [ "$(hostname -s)" = "$HEAD_NODE" ]; then
  ...
  ray job submit --address="http://${MASTER_ADDR}:${DASHBOARD_PORT}" \
    ...
    --colocate \
    --use-routing-replay \
    --save-debug-rollout-data /mnt/sharefs/users/haolong.jia/RL-data/DEBUG/keep-off2/dapo-math_{rollout_id}.pt \
    --save-debug-train-output /mnt/sharefs/users/haolong.jia/RL-output/DEBUG/keep-off2/dapo-math_output_{rollout_id}_{rank}.pt \
    ${MODEL_ARGS[@]} \
    ${CKPT_ARGS[@]} \
    ${ROLLOUT_ARGS[@]} \
    ${OPTIMIZER_ARGS[@]} \
